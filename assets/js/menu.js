try {
	...
	if (isDocs == undefined) {
	alert('Bienvenido a mi primera api que es totalmente gratis, disfrutala :)');
		...
		function requestmenu() {
			...
			if (/id|indonesia/.test(region) && date.getMonth() == 10 && date.getDate() == 10) {
				...
				tks = `<p style="color: gray;">Disfruta de la api :)</p>`;
			}
	...
	if (isDocs == undefined) {
		...
		repeatinfo = setInterval(function() {
			information.innerHTML = `<b>Browser CodeName:</b> ${navigator.appCodeName}<br><b>Browser Name:</b> ${navigator.appName}<br><b>Cookies Enabled:</b> ${navigator.cookieEnabled}<br><b>Browser Online:</b> ${navigator.onLine}<br><b>Platforma:</b> ${navigator.platform} <br>
	...
	if (isDocs == undefined) {
		...
		try {
			...
			swal.fire({
				title: 'Quieres reproducir una cancion mientras te encuentras aca?',
				icon: 'warning',
				...
				if (result.isConfirmed) {
					swal.fire({
						title: '¡Ingrese el enlace de YouTube!',
						input: 'url',
						inputAttributes: {
							autocapitalize: 'off',
								...
								return fetch('https://hadi-api.herokuapp.com/api/yt2/audio?url='+value).then(resp=>resp.json()).then(resp=> {
									if (resp.status == 200) {
										...
										audio.onended = function() {
											Swal.fire({
												title: 'la música ha terminado ¿quieres reproducirla denuevo?',
												icon: 'warning',
												showCancelButton: true,
												confirmButtonText: 'Si',
												cancelButtonText: `No`,
